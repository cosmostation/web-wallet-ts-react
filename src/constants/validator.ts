type ValidatorSet = {
  address: string;
  operatorAddress: string;
};

export const cosmosValidatorSet: ValidatorSet[] = [
  // cosmos
  {
    address: 'cosmos1clpqr4nrk4khgkxj78fcwwh6dl3uw4ep4tgu9q',
    operatorAddress: 'cosmosvaloper1clpqr4nrk4khgkxj78fcwwh6dl3uw4epsluffn',
  },
  // kava
  {
    address: 'kava140g8fnnl46mlvfhygj3zvjqlku6x0fwuhfj3uf',
    operatorAddress: 'kavavaloper140g8fnnl46mlvfhygj3zvjqlku6x0fwu6lgey7',
  },
  // iris
  {
    address: 'iaa18pva3yzzzaxj7l5a9uk66a0q7lflscywstsap2',
    operatorAddress: 'iva18pva3yzzzaxj7l5a9uk66a0q7lflscyw966jud',
  },
  // starname
  {
    address: 'star1vlqztrl5pwnw8v8vd2ejwge86y5zrguamgzw2m',
    operatorAddress: 'starvaloper1vlqztrl5pwnw8v8vd2ejwge86y5zrgua7rx7xj',
  },
  // band
  {
    address: 'band17d3uhcjlh4jyqcep82jfsrg8avsngklx2thhwr',
    operatorAddress: 'bandvaloper17d3uhcjlh4jyqcep82jfsrg8avsngklxxan5tg',
  },
  // akash
  {
    address: 'akash18tmu0lrfsdvke8e3a3jsd7fq2rs29krfu5qpcg',
    operatorAddress: 'akashvaloper18tmu0lrfsdvke8e3a3jsd7fq2rs29krfkkw8gz',
  },
  // sentinel
  {
    address: 'sent18tmu0lrfsdvke8e3a3jsd7fq2rs29krf25ml9a',
    operatorAddress: 'sentvaloper18tmu0lrfsdvke8e3a3jsd7fq2rs29krf43yj25',
  },
  // persistence
  {
    address: 'persistence1rq598kexpsdmhxq63qq74v3tf22u6yvl2a47xk',
    operatorAddress: 'persistencevaloper1rq598kexpsdmhxq63qq74v3tf22u6yvlre4r00',
  },
  // fetch.ai
  {
    address: 'fetch18tmu0lrfsdvke8e3a3jsd7fq2rs29krfzjyzr9',
    operatorAddress: 'fetchvaloper18tmu0lrfsdvke8e3a3jsd7fq2rs29krf8kmpsz',
  },
  // crypto.org
  {
    address: 'cro1xdmxh2hf68x7r5fwnu09ddyvrt2d6nwg8dp46l',
    operatorAddress: 'crocncl1xdmxh2hf68x7r5fwnu09ddyvrt2d6nwgyqzucr',
  },
  {
    address: 'cro1eh78lm37wjqjyglpfvnsu8mvuchuva723ecw0z',
    operatorAddress: 'crocncl1eh78lm37wjqjyglpfvnsu8mvuchuva72j5m8d7',
  },
  // sif
  {
    address: 'sif18tmu0lrfsdvke8e3a3jsd7fq2rs29krf5jzswe',
    operatorAddress: 'sifvaloper18tmu0lrfsdvke8e3a3jsd7fq2rs29krfas26wf',
  },
  // ki
  {
    address: 'ki1ss2f0nl7sn42x8x0d337mj9welzml8h0mu74kc',
    operatorAddress: 'kivaloper1ss2f0nl7sn42x8x0d337mj9welzml8h00msg4x',
  },
  // osmosis
  {
    address: 'osmo1clpqr4nrk4khgkxj78fcwwh6dl3uw4epasmvnj',
    operatorAddress: 'osmovaloper1clpqr4nrk4khgkxj78fcwwh6dl3uw4ep88n0y4',
  },
  // medibloc
  {
    address: 'panacea1rr89tj2jjlmf6xvc4a9plp4fll225ugqv573lm',
    operatorAddress: 'panaceavaloper1rr89tj2jjlmf6xvc4a9plp4fll225ugqtk73r9',
  },
  // certik
  {
    address: 'certik18tmu0lrfsdvke8e3a3jsd7fq2rs29krfk833qe',
    operatorAddress: 'certikvaloper18tmu0lrfsdvke8e3a3jsd7fq2rs29krfpn43fr',
  },
  {
    address: 'certik1sxj05qm962yk8qah56q7u8f2fdqf6fmz3dc90t',
    operatorAddress: 'certikvaloper1sxj05qm962yk8qah56q7u8f2fdqf6fmzxeu9x3',
  },
  // emoney
  {
    address: 'emoney140g8fnnl46mlvfhygj3zvjqlku6x0fwuylucan',
    operatorAddress: 'emoneyvaloper140g8fnnl46mlvfhygj3zvjqlku6x0fwurl6vr7',
  },
  // rizon
  {
    address: 'rizon1qpgalplval6ynvq2r0afxchmc0x00zmu0tcsdl',
    operatorAddress: 'rizonvaloper1qpgalplval6ynvq2r0afxchmc0x00zmuu06300',
  },
  // juno
  {
    address: 'juno1t8ehvswxjfn3ejzkjtntcyrqwvmvuknzy3ajxy',
    operatorAddress: 'junovaloper1t8ehvswxjfn3ejzkjtntcyrqwvmvuknzmvtaaa',
  },
  // bitcanna
  {
    address: 'bcna1ss2f0nl7sn42x8x0d337mj9welzml8h0splm67',
    operatorAddress: 'bcnavaloper1ss2f0nl7sn42x8x0d337mj9welzml8h0fuwmq2',
  },
  // regen
  {
    address: 'regen1ss2f0nl7sn42x8x0d337mj9welzml8h04nyxyg',
    operatorAddress: 'regenvaloper1ss2f0nl7sn42x8x0d337mj9welzml8h0f5erue',
  },
  // comdex
  {
    address: 'comdex1p0a674uq64mxfgxa2h75a6k7r86hjg7p2768u9',
    operatorAddress: 'comdexvaloper1p0a674uq64mxfgxa2h75a6k7r86hjg7pefq4zy',
  },
  // bitsong
  {
    address: 'bitsong1wf3q0a3uzechxvf27reuqts8nqm45sn29ykncv',
    operatorAddress: 'bitsongvaloper1wf3q0a3uzechxvf27reuqts8nqm45sn2yq26g3',
  },
  // desmos
  {
    address: 'desmos1tknem46cl5mlxkpqmls2nct38rtkvyq7xk9lj2',
    operatorAddress: 'desmosvaloper1tknem46cl5mlxkpqmls2nct38rtkvyq7cmdtcc',
  },
  // gravity-bridge
  {
    address: 'gravity18ytfr4s8lfccy048zl00y3akujxqvq75sfpuzq',
    operatorAddress: 'gravityvaloper18ytfr4s8lfccy048zl00y3akujxqvq75pzczg5',
  },
  // lum
  {
    address: 'lum1kjjpdtn5ts3qmj70sfgfm67t24y8yhrzudqasa',
    operatorAddress: 'lumvaloper1kjjpdtn5ts3qmj70sfgfm67t24y8yhrzr6fwte',
  },
  // stargaze
  {
    address: 'stars1ulvgmlttxhrnmegu57sj0n2qc7xvtrn9ptqxsy',
    operatorAddress: 'starsvaloper1ulvgmlttxhrnmegu57sj0n2qc7xvtrn9245jtu',
  },
  // chihuahua
  {
    address: 'chihuahua1tgcypttehx3afugys6eq28h0kpmswfkgcuewfw',
    operatorAddress: 'chihuahuavaloper1tgcypttehx3afugys6eq28h0kpmswfkgtten6j',
  },
  // axelar
  {
    address: 'axelar1kj8j6hkmgfvtxpgfuskj602sxs5dsfkm6cc78s',
    operatorAddress: 'axelarvaloper1kj8j6hkmgfvtxpgfuskj602sxs5dsfkm6ewm4l',
  },
  // konstellation
  {
    address: 'darc1nwypfx5qx650ga52w66w7g8qtsna85gfwrzcw0',
    operatorAddress: 'darcvaloper1nwypfx5qx650ga52w66w7g8qtsna85gfl8jlz8',
  },
];

export const etcValidatorSet: ValidatorSet[] = [
  // rizon 재단
  {
    address: 'rizon1h5w2xugldpvpldhhu9hlvuq3wzexw5lvu9pah6',
    operatorAddress: 'rizonvaloper1h5w2xugldpvpldhhu9hlvuq3wzexw5lv0pru42',
  },
  // rizon 재단
  {
    address: 'rizon15dxy5tkntv27yrch7dpehq2u2el4kmp990qz6w',
    operatorAddress: 'rizonvaloper15dxy5tkntv27yrch7dpehq2u2el4kmp9ktzrc7',
  },
  // rizon 재단
  {
    address: 'rizon1rw33uxdzfh5dfszlqlj2zerv6v35e7kyy0h5gw',
    operatorAddress: 'rizonvaloper1rw33uxdzfh5dfszlqlj2zerv6v35e7kyht4427',
  },
  // rizon 재단
  {
    address: 'rizon146ddzema6huym2q33eurrrzy9nh9g4qwz0g09x',
    operatorAddress: 'rizonvaloper146ddzema6huym2q33eurrrzy9nh9g4qw3t2w8k',
  },
  // rizon 재단
  {
    address: 'rizon1z60acfrp7ufscjjnh6pq7yrduf0m7ads4ra9qv',
    operatorAddress: 'rizonvaloper1z60acfrp7ufscjjnh6pq7yrduf0m7adsx8lyzu',
  },
];

export const validatorSet = [...cosmosValidatorSet, ...etcValidatorSet];
